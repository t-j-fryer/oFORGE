...
class DefaultAndPathTests(unittest.TestCase):
    ...
    def test_readme_workflow_images_and_shared_template_protocol(self) -> None:
        ...
        self.assertIn('--overhangs "/path/to/custom_overhangs.csv"', readme)
        self.assertIn("produces final synthesis oligos that are 350 nt long", readme)
        self.assertIn("assigns exactly one gene to each sub-pool", readme)
